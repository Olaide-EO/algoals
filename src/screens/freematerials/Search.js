...
const Search = ({ route, navigation }) => {
  ...
  const currentFreeMaterialsArray = useSelector(
    (state) => state.freeMaterials[currentScreen].data
  );
  ...
  useEffect(() => {
    setSearcher(
      new FuzzySearch(currentFreeMaterialsArray, ["title"], {
        caseSensitive: false,
        sort: true,
      })
    );
  }, [currentFreeMaterialsArray]);
  ...
  const setFreeMaterialIndex = (index, title) => {
    dispatch({
  ...
  const navigateAfterPress = () => {
    navigation.navigate("FreeMaterials", {
      screen: titleReducer(currentScreen)
    })
  };

  const onPress = async (index, title) => {
    await setFreeMaterialIndex(index, title);
